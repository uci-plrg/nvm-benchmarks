...
class btree{
    ...
    public:
        ...
        bool btree_insert(uint64_t, char*);
        bool btree_insert(char*, char*);
...
class entry{
    ...
    public :
        entry(){
            ...
            ptr = NULL;
#ifdef BUGFIX
    //   clflush((char*)this, sizeof(entry), false, true);
#endif
        }
...
#ifdef MT_CRASH_MERGE_FF
                if (simulateCrash){
                    int shouldCrash = rand()%2;
                    if(shouldCrash){
                        std::cout << "\tDeletion of "<< key << " abruptly terminated due to merge" << std::endl;
                        lock_initialization();
                        return true;
                    }
                }
...
#ifdef MT_CRASH_SPLIT_FF
                if (simulateCrash){
                    int shouldCrash = rand()%2;
                    if(shouldCrash){
                        std::cout << "\tInsertion of "<< key << " abruptly terminated during split" << std::endl;
                        lock_initialization();
                        return (page*)DUMMY_PTR;
                    }
                }
#endif
...
btree::btree(){
    root = (char*)new page();
#ifdef BUGFIX
    clflush((char*)root, sizeof(page)); //b1 ...requires -f option
#endif
    height = 1;
#ifdef BUGFIX
    clflush((char*)this, sizeof(btree), false, true);//b2
#endif
}
...
bool btree::btree_insert(uint64_t key, char* right){ //need to be string
    page* p = (page*)root;
	...
	page * ret =  p->store(this, NULL, key, right, true, true);

	// if CRASH
	if (ret == (page *) DUMMY_PTR)
		return false;	
...
// insert the key in the leaf node
bool btree::btree_insert(char *key, char* right){ //need to be string
    page* p = (page*)root;
    ...
    page * ret =  p->store(this, NULL, new_item, right, true, true);
