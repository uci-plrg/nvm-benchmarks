...
#include "util/hash.h"
#include "src/CCEH-crash.h"
#include <sys/types.h>
#include <sys/wait.h>
#include <unistd.h>
#include <fstream>

extern bool simulateCrash;

#ifdef CCEHDEBUG
    std::ostream &cceh_cout = std::cout;
#else
    std::ofstream dev_null_cceh("/dev/null");
    std::ostream &cceh_cout = dev_null_cceh;
#endif
...
int Segment::Insert(Key_t& key, Value_t value, size_t loc, size_t key_hash) {
  ...
  if ((key_hash >> (8*sizeof(key_hash)-local_depth)) != pattern) {
		cceh_cout << "returning due to key_has" << std::endl;
		return 2;
  }
  auto lock = sema;
  int ret = 1;
...
RETRY:
  ...
  cceh_cout << __func__ <<  " Seg insert returend " << ret << std::endl;
  if (ret == 1) {
    timer.Start();
    ...
    { // CRITICAL SECTION - directory update
      ...
      if (dir._[x]->local_depth < global_depth) {  // normal split
		...
		cceh_cout << __func__ << " Dir doubling " << std::endl;
        auto d = dir._;
...
#ifdef MT_CRASH_AFTER_DD_STEP_1
        if (simulateCrash){
            int shouldCrash = rand()%2;
            if(shouldCrash){
                std::cout << "\tInsertion  abruptly terminated : Dir doubling step 1" << std::endl;
                lock_initialization();
				Recovery();
                return false;
            }
        ...
        if (pid == 0) {
            ...
            lock_initialization();
            Recovery();
            cceh_cout << __func__ << " Child crashed after dir doubling step 1" << std::endl;
            return false;
        }
            ...
            cceh_cout << __func__ << " Continuing in parent process to DD step 2 " << std::endl;
        }
        else {
            cceh_cout << __func__ << "Fork failed" << std::endl;
            return false;
        }
#endif
...
#ifdef MT_CRASH_AFTER_DD_STEP_2
        if (simulateCrash){
            int shouldCrash = rand()%2;
            if(shouldCrash){
                std::cout << "\tInsertion  abruptly terminated : Dir doubling step 2" << std::endl;
                lock_initialization();
                Recovery();
                return false;
            }
        ...
        if (pid == 0) {
            ...
            lock_initialization();
			Recovery();
            cceh_cout << __func__ << " Child crashed after dir doubling step 2" << std::endl;
            return false;
        }
            ...
            int returnStatus;
            waitpid(pid, &returnStatus, 0);
            cceh_cout << __func__ << " Continuing in parent process to DD step 3 " << std::endl;
        }
        else {
            cceh_cout << __func__ << "Fork failed" << std::endl;
            return false;
...
#ifdef MT_CRASH_AFTER_DD_STEP_3
        if (simulateCrash){
            int shouldCrash = rand()%2;
            if(shouldCrash){
                std::cout << "\tInsertion  abruptly terminated : Dir doubling step 3" << std::endl;
                lock_initialization();
                Recovery();
                return false
            }
...
#ifdef CRASH_AFTER_DD_STEP_3
        pid_t pid1 = fork();

        if (pid1 == 0) {
            lock_initialization();
            Recovery();
            cceh_cout << __func__ << " Child crashed after dir doubling step 3" << std::endl;
            return false;
        }
            ...
            cceh_cout << __func__ << " Continuing in parent process to finish dir doubling " << std::endl;
        }
        else {
            cceh_cout << __func__ << "Fork failed" << std::endl;
            return false;
...
bool CCEH::Recovery(void) {
  cceh_cout << __func__ << " Performing RECOVERY for cap " << dir.capacity << std::endl;
