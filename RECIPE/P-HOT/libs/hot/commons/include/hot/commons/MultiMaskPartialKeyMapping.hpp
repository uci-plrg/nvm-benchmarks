...
//#1U Class Definitions

inline typename MultiMaskPartialKeyMapping<1u>::SIMDRegisterType
MultiMaskPartialKeyMapping<1u>::getPositionsRegister() const {
	return SIMDHelperType::toRegister(mExtractionPositions);
}

inline typename MultiMaskPartialKeyMapping<1u>::SIMDRegisterType
MultiMaskPartialKeyMapping<1u>::getExtractionDataRegister() const {
	return SIMDHelperType::toRegister(mExtractionData);
}

inline void
MultiMaskPartialKeyMapping<1u>::setPositions(typename MultiMaskPartialKeyMapping<1u>::SIMDRegisterType positions) {
	return SIMDHelperType::store(positions, mExtractionPositions);
}

inline void
MultiMaskPartialKeyMapping<1u>::setExtractionData(typename MultiMaskPartialKeyMapping<1u>::SIMDRegisterType extractionData) {
	return SIMDHelperType::store(extractionData, mExtractionData);
}

inline MultiMaskPartialKeyMapping<1u>::MultiMaskPartialKeyMapping(MultiMaskPartialKeyMapping<1u> const & src)
...
inline MultiMaskPartialKeyMapping<1u>::MultiMaskPartialKeyMapping(
	SingleMaskPartialKeyMapping const & existing, DiscriminativeBit const & significantKeyInformation
) : PartialKeyMappingBase(existing, significantKeyInformation)
{
	initializeDataAndPositionsWithZero();
...
//detect insert position -> add byte|position -> finish
inline MultiMaskPartialKeyMapping<1u>::MultiMaskPartialKeyMapping(
	MultiMaskPartialKeyMapping<1u> const & existing, DiscriminativeBit const & significantKeyInformation, unsigned int extractionByteIndex
)  : MultiMaskPartialKeyMapping(existing)
{
	//this is a performance optimization to not call the BaseDiscriminativeBitsRepresentation constructor and use copying
	mMostSignificantDiscriminativeBitIndex = std::min(mMostSignificantDiscriminativeBitIndex, significantKeyInformation.mAbsoluteBitIndex);
...
inline MultiMaskPartialKeyMapping<1u>::MultiMaskPartialKeyMapping(
	...
	uint16_t const mostSignificantBitIndex, uint16_t const leastSignificantBitIndex,
	uint16_t const numberBytesUsed, uint16_t const bitsUsed
) : MultiMaskPartialKeyMapping<1u>(mostSignificantBitIndex, leastSignificantBitIndex, numberBytesUsed, bitsUsed) {
	assert(_mm_popcnt_u32(bitsUsed) >= 1);
...
inline __attribute__((always_inline)) uint32_t MultiMaskPartialKeyMapping<1u>::extractMask(uint8_t const * keyBytes) const {
	return extractMaskForMappedInput(mapInput(keyBytes));
}
 ...
 * Extracts a bit masks corresponding to 111111111|00000000 with the first zero bit marking the misssmatching bit index.
 */
template<typename PartialKeyType> inline PartialKeyType MultiMaskPartialKeyMapping<1u>::getPrefixBitsMask(DiscriminativeBit const &significantKeyInformation) const {
	//initialized to zero
	ExtractionDataArray prefixBits = zeroInitializedArray();
	uint8_t subtreeMissmatchingByte = static_cast<uint8_t >(UINT8_MAX) << (8 - significantKeyInformation.mByteRelativeBitIndex);
...
inline uint16_t MultiMaskPartialKeyMapping<1u>::calculateNumberBitsUsed() const {
	return mNumberKeyBits;
}

template<typename Operation> inline auto MultiMaskPartialKeyMapping<1u>::insert(DiscriminativeBit const & significantKeyInformation, Operation const & operation) const {
	SIMDRegisterType simdMask = getMaskForPositionsLargerOrEqualTo(significantKeyInformation.mByteIndex);
	//| (UINT32_MAX << mNumberExtractionBytes) is important here. it corresponds to all bytes which are currently unused. Hence, they are expected to be larger
	uint32_t maskForPositionsLargerOrEqual = SIMDHelperType::moveMask8(simdMask) | (UINT32_MAX << mNumberExtractionBytes);
	unsigned int possibleExtractionByteIndex = __tzcnt_u32(maskForPositionsLargerOrEqual);
	...
	constexpr unsigned int nextNumberExtractionMasks = 1u == 4 ? 1u : 1u * 2;
	return (isByteContainedAtPosition)
		? operation(MultiMaskPartialKeyMapping<1u>(*this, significantKeyInformation, possibleExtractionByteIndex))
		: (mNumberExtractionBytes < (1u * 8)
		   ? operation(MultiMaskPartialKeyMapping<1u>(*this, significantKeyInformation, possibleExtractionByteIndex, simdMask))
		   : operation(MultiMaskPartialKeyMapping<nextNumberExtractionMasks>(*this, significantKeyInformation, possibleExtractionByteIndex, simdMask))
		  );
}

template<typename Operation>
inline auto MultiMaskPartialKeyMapping<1u>::extract(uint32_t bitsUsed, Operation const &operation) const {
	assert(_mm_popcnt_u32(bitsUsed) >= 1);
	...
	assert((numberRandomExtractionMasksMinusOne + isRandomExtractionMask) <= 1u);
	assert((numberRandomExtractionMasksMinusOne + isRandomExtractionMask) <= 4);
	switch(numberRandomExtractionMasksMinusOne + isRandomExtractionMask) {
		case 0:
			return operation(SingleMaskPartialKeyMapping(
		...
		case 1:
			return operation(MultiMaskPartialKeyMapping<1u>(
				(MultiMaskPartialKeyMapping<1u> const &)(*this), bytesUsedMask, extractionDataForBitsUsed,
				mostSignificantBitPosition, leastSignificantBitPosition, numberBytesUsed, static_cast<uint16_t>(_mm_popcnt_u32(bitsUsed))
			));
		case 2:
			if(1u >= 2) {
...
inline std::array<uint8_t, 256> MultiMaskPartialKeyMapping<1u>::createIntermediateKeyWithOnlySignificantBitsSet() const {
	std::array<uint8_t, 256> intermediateKey;
	std::memset(intermediateKey.data(), 0, 256);
	for(size_t i=0u; i < mNumberExtractionBytes; ++i) {
		intermediateKey[getExtractionBytePosition(i)] = getExtractionByte(i);
...
template<typename Operation> inline auto MultiMaskPartialKeyMapping<1u>::executeWithCorrectMaskAndDiscriminativeBitsRepresentation(Operation const & operation) const {
...
template<typename Operation> inline auto MultiMaskPartialKeyMapping<1u>::executeWithCompressedDiscriminativeBitsRepresentation(Operation const & operation) const {
	uint8_t leasSignificantExtractionBytePosition = getExtractionBytePosition(getNumberExtractionBytes() - 1);
	uint8_t mostSignificantExtractionBytePosition = getExtractionBytePosition(0);
	size_t extractionByteRange = leasSignificantExtractionBytePosition - mostSignificantExtractionBytePosition;
	if(extractionByteRange < 8) {
...
inline __m64 MultiMaskPartialKeyMapping<1u>::getMaskForPositionsLargerOrEqualTo(unsigned int bytePosition) const {
...
inline void MultiMaskPartialKeyMapping<1u>::initializeDataAndPositionsWithZero() {
	mExtractionPositions[0] = 0;
	mExtractionData[0] = 0;
}


inline typename MultiMaskPartialKeyMapping<1u>::ExtractionDataArray MultiMaskPartialKeyMapping<1u>::mapInput(uint8_t const* __restrict__ keyBytes) const {
	ExtractionDataArray mappedInput = zeroInitializedArray();
	uint8_t* __restrict__ mappedInputBytes = reinterpret_cast<uint8_t*>(mappedInput.data());
	uint8_t const * __restrict__ positions = reinterpret_cast<uint8_t const* >(mExtractionPositions.data());
	for(int i=0; i < mNumberExtractionBytes; ++i) {
...
inline __attribute__((always_inline)) uint32_t MultiMaskPartialKeyMapping<1u>::extractMaskForMappedInput(typename MultiMaskPartialKeyMapping<1u>::ExtractionDataArray const & mappedInputData) const {
	return _pext_u64(mappedInputData[0], mExtractionData[0]);
}

inline uint32_t  MultiMaskPartialKeyMapping<1u>::getMaskForHighestBit() const {
	unsigned int byteRelativeBitPositionForHighestBit = mMostSignificantDiscriminativeBitIndex - convertBytesToBits(getExtractionBytePosition(0));
...
inline uint32_t  MultiMaskPartialKeyMapping<1u>::getMaskFor(DiscriminativeBit const & significantKeyInformation) const {
	ExtractionDataArray extractionData = zeroInitializedArray();
	setExtractionByteAt(
		extractionData,
		getExtractionByteIndexForPosition(significantKeyInformation.mByteIndex),
...
inline uint32_t  MultiMaskPartialKeyMapping<1u>::getAllMaskBits() const {
	uint32_t numberUnusedBits = (32 - mNumberKeyBits);
	return UINT32_MAX >> numberUnusedBits;
}

inline uint32_t  MultiMaskPartialKeyMapping<1u>::getExtractionByteIndexForPosition(uint16_t bytePosition) const {
	SIMDRegisterType needle = SIMDHelperType::set1_epi8(bytePosition);
	SIMDRegisterType haystack = getPositionsRegister();
	uint32_t bytePositionMask = SIMDHelperType::moveMask8(SIMDHelperType::cmpeq_epi8(needle, haystack)) & getMaskForExtractionBytesUsed();
	assert(bytePositionMask != 0);
...
inline uint8_t  MultiMaskPartialKeyMapping<1u>::getExtractionByteWithBitSetAtRelativePosition(uint16_t byteRelativeBitPosition) {
	return BYTE_WITH_HIGHEST_BIT_SET >> byteRelativeBitPosition;
}

inline typename  MultiMaskPartialKeyMapping<1u>::ExtractionDataArray MultiMaskPartialKeyMapping<1u>::getUsedExtractionBitsForMask(uint32_t usedMaskBits) const {
	return { _pdep_u64(usedMaskBits, mExtractionData[0]) };
}


inline typename  MultiMaskPartialKeyMapping<1u>::ExtractionDataArray MultiMaskPartialKeyMapping<1u>::zeroInitializedArray() {
	ExtractionDataArray data;
	data[0] = 0ul;
	return std::move(data);
}

inline uint8_t MultiMaskPartialKeyMapping<1u>::getExtractionByteAt(typename MultiMaskPartialKeyMapping<1u>::ExtractionDataArray const &extractionData, uint32_t extractionByteIndex) {
	return reinterpret_cast<uint8_t const *>(extractionData.data())[extractionByteIndex];
}


inline void MultiMaskPartialKeyMapping<1u>::setExtractionByteAt(typename MultiMaskPartialKeyMapping<1u>::ExtractionDataArray  & extractionData, uint32_t extractionByteIndex, uint8_t extractionByte) {
	reinterpret_cast<uint8_t *>(extractionData.data())[extractionByteIndex] = extractionByte;
}

inline uint32_t MultiMaskPartialKeyMapping<1u>::getMaskForExtractionBytesUsed() const {
	return (UINT32_MAX >> (32 - mNumberExtractionBytes));
}

inline uint32_t MultiMaskPartialKeyMapping<1u>::getBytesUsedMaskForExtractionData(typename MultiMaskPartialKeyMapping<1u>::ExtractionDataArray const & extractionData) const {
	return ~SIMDHelperType::moveMask8(SIMDHelperType::cmpeq_epi8(SIMDHelperType::toRegister(extractionData), SIMDHelperType::zero())) & getMaskForExtractionBytesUsed();
}

inline uint32_t MultiMaskPartialKeyMapping<1u>::getLeastSignificantBytIndexForBytesUsedMask(uint32_t bytesUsedMask) const {
	return 31 - _lzcnt_u32(bytesUsedMask);
}

template<typename PartialKeyType> inline PartialKeyType MultiMaskPartialKeyMapping<1u>::getMostSignifikantMaskBit(PartialKeyType mask) const {
	ExtractionDataArray usedExtractionBits = getUsedExtractionBitsForMask(mask);
	uint32_t usedExtractionBytesMask = getBytesUsedMaskForExtractionData(usedExtractionBits);
	uint32_t highestExtractionByteIndex = __tzcnt_u32(usedExtractionBytesMask);
	uint8_t highestByte = getExtractionByteAt(usedExtractionBits, highestExtractionByteIndex);
...
inline uint16_t MultiMaskPartialKeyMapping<1u>::getLeastSignificantBitIndex(uint32_t mask) const {
	ExtractionDataArray usedExtractionBits = getUsedExtractionBitsForMask(mask);
	uint32_t lowestExtractionByteIndex = getLeastSignificantBytIndexForBytesUsedMask(getBytesUsedMaskForExtractionData(usedExtractionBits));
	uint8_t lowestByte = getExtractionByteAt(usedExtractionBits, lowestExtractionByteIndex);
	return convertBytesToBits(getExtractionBytePosition(lowestExtractionByteIndex)) + getLeastSignificantBitIndexInByte(lowestByte);
...
inline uint8_t MultiMaskPartialKeyMapping<1u>::getExtractionBytePosition(unsigned int index) const {
	return reinterpret_cast<uint8_t const *>(mExtractionPositions.data())[index];
}

inline void MultiMaskPartialKeyMapping<1u>::setExtractionBytePosition(unsigned int index, uint8_t extractionByte) {
	reinterpret_cast<uint8_t *>(mExtractionPositions.data())[index] = extractionByte;
}

inline uint8_t MultiMaskPartialKeyMapping<1u>::getExtractionByte(unsigned int index) const {
	return reinterpret_cast<uint8_t const *>(mExtractionData.data())[index];
}

inline void MultiMaskPartialKeyMapping<1u>::setExtractionByte(unsigned int index, uint8_t extractionByte) {
	reinterpret_cast<uint8_t*>(mExtractionData.data())[index] = extractionByte;
}

inline std::set<uint16_t> MultiMaskPartialKeyMapping<1u>::getDiscriminativeBits() const {
	std::set<uint16_t> extractionBits;
	for(int i = 0; i < mNumberExtractionBytes; ++i) {
		unsigned int extractionBasePosition = convertBytesToBits(getExtractionBytePosition(i));
		uint32_t extractionByte = getExtractionByte(i);
...
inline uint16_t MultiMaskPartialKeyMapping<1u>::getNumberExtractionBytes() const {
	return mNumberExtractionBytes;
}

//2u Class Definitions----------------------

inline typename MultiMaskPartialKeyMapping<2u>::SIMDRegisterType
MultiMaskPartialKeyMapping<2u>::getPositionsRegister() const {
	return SIMDHelperType::toRegister(mExtractionPositions);
}

inline typename MultiMaskPartialKeyMapping<2u>::SIMDRegisterType
MultiMaskPartialKeyMapping<2u>::getExtractionDataRegister() const {
	return SIMDHelperType::toRegister(mExtractionData);
}

inline void
MultiMaskPartialKeyMapping<2u>::setPositions(typename MultiMaskPartialKeyMapping<2u>::SIMDRegisterType positions) {
	return SIMDHelperType::store(positions, mExtractionPositions);
}

inline void
MultiMaskPartialKeyMapping<2u>::setExtractionData(typename MultiMaskPartialKeyMapping<2u>::SIMDRegisterType extractionData) {
	return SIMDHelperType::store(extractionData, mExtractionData);
}

inline MultiMaskPartialKeyMapping<2u>::MultiMaskPartialKeyMapping(MultiMaskPartialKeyMapping<2u> const & src)
	: PartialKeyMappingBase()
{
	*reinterpret_cast<uint64_t*>(this) = *reinterpret_cast<uint64_t const *>(&src);
...
inline MultiMaskPartialKeyMapping<2u>::MultiMaskPartialKeyMapping(MultiMaskPartialKeyMapping<1u> const & src)
	: PartialKeyMappingBase()
{
	*reinterpret_cast<uint64_t*>(this) = *reinterpret_cast<uint64_t const *>(&src);
...
inline MultiMaskPartialKeyMapping<2u>::MultiMaskPartialKeyMapping(
	SingleMaskPartialKeyMapping const & existing, DiscriminativeBit const & significantKeyInformation
) : PartialKeyMappingBase(existing, significantKeyInformation)
{
	initializeDataAndPositionsWithZero();
...
//detect insert position -> add byte|position -> finish
inline MultiMaskPartialKeyMapping<2u>::MultiMaskPartialKeyMapping(
	MultiMaskPartialKeyMapping<2u> const & existing, DiscriminativeBit const & significantKeyInformation, unsigned int extractionByteIndex
)  : MultiMaskPartialKeyMapping(existing)
{
	//this is a performance optimization to not call the BaseDiscriminativeBitsRepresentation constructor and use copying
	mMostSignificantDiscriminativeBitIndex = std::min(mMostSignificantDiscriminativeBitIndex, significantKeyInformation.mAbsoluteBitIndex);
...
template<> inline MultiMaskPartialKeyMapping<2u>::MultiMaskPartialKeyMapping(
	MultiMaskPartialKeyMapping<1u> const & existing, DiscriminativeBit const & significantKeyInformation, unsigned int const extractionByteIndex,
	typename MultiMaskPartialKeyMapping<1u>::SIMDRegisterType const & maskForLessSignificantBytes
...
inline MultiMaskPartialKeyMapping<2u>::MultiMaskPartialKeyMapping(
	...
	uint16_t const mostSignificantBitIndex, uint16_t const leastSignificantBitIndex,
	uint16_t const numberBytesUsed, uint16_t const bitsUsed
) : MultiMaskPartialKeyMapping<2u>(mostSignificantBitIndex, leastSignificantBitIndex, numberBytesUsed, bitsUsed) {
	assert(_mm_popcnt_u32(bitsUsed) >= 1);
...
inline __attribute__((always_inline)) uint32_t MultiMaskPartialKeyMapping<2u>::extractMask(uint8_t const * keyBytes) const {
	return extractMaskForMappedInput(mapInput(keyBytes));
}
 ...
 * Extracts a bit masks corresponding to 111111111|00000000 with the first zero bit marking the misssmatching bit index.
 */
template<typename PartialKeyType> inline PartialKeyType MultiMaskPartialKeyMapping<2u>::getPrefixBitsMask(DiscriminativeBit const &significantKeyInformation) const {
	//initialized to zero
	ExtractionDataArray prefixBits = zeroInitializedArray();
	uint8_t subtreeMissmatchingByte = static_cast<uint8_t >(UINT8_MAX) << (8 - significantKeyInformation.mByteRelativeBitIndex);
...
inline uint16_t MultiMaskPartialKeyMapping<2u>::calculateNumberBitsUsed() const {
	return mNumberKeyBits;
}

template<typename Operation> inline auto MultiMaskPartialKeyMapping<2u>::insert(DiscriminativeBit const & significantKeyInformation, Operation const & operation) const {
	SIMDRegisterType simdMask = getMaskForPositionsLargerOrEqualTo(significantKeyInformation.mByteIndex);
	//| (UINT32_MAX << mNumberExtractionBytes) is important here. it corresponds to all bytes which are currently unused. Hence, they are expected to be larger
	uint32_t maskForPositionsLargerOrEqual = SIMDHelperType::moveMask8(simdMask) | (UINT32_MAX << mNumberExtractionBytes);
	unsigned int possibleExtractionByteIndex = __tzcnt_u32(maskForPositionsLargerOrEqual);
	...
	constexpr unsigned int nextNumberExtractionMasks = 2u == 4 ? 2u : 2u * 2;
	return (isByteContainedAtPosition)
		? operation(MultiMaskPartialKeyMapping<2u>(*this, significantKeyInformation, possibleExtractionByteIndex))
		: (mNumberExtractionBytes < (2u * 8)
		   ? operation(MultiMaskPartialKeyMapping<2u>(*this, significantKeyInformation, possibleExtractionByteIndex, simdMask))
		   : operation(MultiMaskPartialKeyMapping<nextNumberExtractionMasks>(*this, significantKeyInformation, possibleExtractionByteIndex, simdMask))
		  );
}

template<typename Operation>
inline auto MultiMaskPartialKeyMapping<2u>::extract(uint32_t bitsUsed, Operation const &operation) const {
	assert(_mm_popcnt_u32(bitsUsed) >= 1);
	...
	assert((numberRandomExtractionMasksMinusOne + isRandomExtractionMask) <= 2u);
	assert((numberRandomExtractionMasksMinusOne + isRandomExtractionMask) <= 4);
	switch(numberRandomExtractionMasksMinusOne + isRandomExtractionMask) {
		case 0:
			return operation(SingleMaskPartialKeyMapping(
		...
		case 1:
			return operation(MultiMaskPartialKeyMapping<2u>(
				(MultiMaskPartialKeyMapping<2u> const &)(*this), bytesUsedMask, extractionDataForBitsUsed,
				mostSignificantBitPosition, leastSignificantBitPosition, numberBytesUsed, static_cast<uint16_t>(_mm_popcnt_u32(bitsUsed))
			));
		case 2:
			if(2u >= 2) {
...
inline std::array<uint8_t, 256> MultiMaskPartialKeyMapping<2u>::createIntermediateKeyWithOnlySignificantBitsSet() const {
	std::array<uint8_t, 256> intermediateKey;
	std::memset(intermediateKey.data(), 0, 256);
	for(size_t i=0u; i < mNumberExtractionBytes; ++i) {
		intermediateKey[getExtractionBytePosition(i)] = getExtractionByte(i);
...
template<typename Operation> inline auto MultiMaskPartialKeyMapping<2u>::executeWithCorrectMaskAndDiscriminativeBitsRepresentation(Operation const & operation) const {
...
template<typename Operation> inline auto MultiMaskPartialKeyMapping<2u>::executeWithCompressedDiscriminativeBitsRepresentation(Operation const & operation) const {
	uint8_t leasSignificantExtractionBytePosition = getExtractionBytePosition(getNumberExtractionBytes() - 1);
	uint8_t mostSignificantExtractionBytePosition = getExtractionBytePosition(0);
	size_t extractionByteRange = leasSignificantExtractionBytePosition - mostSignificantExtractionBytePosition;
	if(extractionByteRange < 8) {
...
inline __m128i MultiMaskPartialKeyMapping<2u>::getMaskForPositionsLargerOrEqualTo(unsigned int bytePosition) const {
...
inline void MultiMaskPartialKeyMapping<2u>::initializeDataAndPositionsWithZero() {
	_mm256_storeu_si256(reinterpret_cast<__m256i*>(mExtractionPositions.data()), _mm256_setzero_si256());
}

inline typename MultiMaskPartialKeyMapping<2u>::ExtractionDataArray MultiMaskPartialKeyMapping<2u>::mapInput(uint8_t const* __restrict__ keyBytes) const {
	ExtractionDataArray mappedInput = zeroInitializedArray();
	uint8_t* __restrict__ mappedInputBytes = reinterpret_cast<uint8_t*>(mappedInput.data());
	uint8_t const * __restrict__ positions = reinterpret_cast<uint8_t const* >(mExtractionPositions.data());
	for(int i=0; i < mNumberExtractionBytes; ++i) {
...
inline __attribute__((always_inline)) uint32_t MultiMaskPartialKeyMapping<2u>::extractMaskForMappedInput(typename MultiMaskPartialKeyMapping<2u>::ExtractionDataArray const & mappedInputData) const {
...
inline uint32_t  MultiMaskPartialKeyMapping<2u>::getMaskForHighestBit() const {
	unsigned int byteRelativeBitPositionForHighestBit = mMostSignificantDiscriminativeBitIndex - convertBytesToBits(getExtractionBytePosition(0));
...
inline uint32_t MultiMaskPartialKeyMapping<2u>::getMaskFor(DiscriminativeBit const & significantKeyInformation) const {
	ExtractionDataArray extractionData = zeroInitializedArray();
	setExtractionByteAt(
		extractionData,
		getExtractionByteIndexForPosition(significantKeyInformation.mByteIndex),
...
inline uint32_t MultiMaskPartialKeyMapping<2u>::getAllMaskBits() const {
	uint32_t numberUnusedBits = (32 - mNumberKeyBits);
	return UINT32_MAX >> numberUnusedBits;
}

inline uint32_t  MultiMaskPartialKeyMapping<2u>::getExtractionByteIndexForPosition(uint16_t bytePosition) const {
	SIMDRegisterType needle = SIMDHelperType::set1_epi8(bytePosition);
	SIMDRegisterType haystack = getPositionsRegister();
	uint32_t bytePositionMask = SIMDHelperType::moveMask8(SIMDHelperType::cmpeq_epi8(needle, haystack)) & getMaskForExtractionBytesUsed();
	assert(bytePositionMask != 0);
...
inline uint8_t  MultiMaskPartialKeyMapping<2u>::getExtractionByteWithBitSetAtRelativePosition(uint16_t byteRelativeBitPosition) {
	return BYTE_WITH_HIGHEST_BIT_SET >> byteRelativeBitPosition;
}

inline typename MultiMaskPartialKeyMapping<2u>::ExtractionDataArray MultiMaskPartialKeyMapping<2u>::getUsedExtractionBitsForMask(uint32_t usedMaskBits) const {
...
inline typename MultiMaskPartialKeyMapping<2u>::ExtractionDataArray MultiMaskPartialKeyMapping<2u>::zeroInitializedArray() {
	ExtractionDataArray data;
	data[0] = 0ul;
	data[1] = 0ul;
	return std::move(data);
...
inline uint8_t MultiMaskPartialKeyMapping<2u>::getExtractionByteAt(typename MultiMaskPartialKeyMapping<2u>::ExtractionDataArray const &extractionData, uint32_t extractionByteIndex) {
	return reinterpret_cast<uint8_t const *>(extractionData.data())[extractionByteIndex];
}


inline void MultiMaskPartialKeyMapping<2u>::setExtractionByteAt(typename MultiMaskPartialKeyMapping<2u>::ExtractionDataArray  & extractionData, uint32_t extractionByteIndex, uint8_t extractionByte) {
	reinterpret_cast<uint8_t *>(extractionData.data())[extractionByteIndex] = extractionByte;
}

inline uint32_t MultiMaskPartialKeyMapping<2u>::getMaskForExtractionBytesUsed() const {
	return (UINT32_MAX >> (32 - mNumberExtractionBytes));
}

inline uint32_t MultiMaskPartialKeyMapping<2u>::getBytesUsedMaskForExtractionData(typename MultiMaskPartialKeyMapping<2u>::ExtractionDataArray const & extractionData) const {
	return ~SIMDHelperType::moveMask8(SIMDHelperType::cmpeq_epi8(SIMDHelperType::toRegister(extractionData), SIMDHelperType::zero())) & getMaskForExtractionBytesUsed();
}

inline uint32_t MultiMaskPartialKeyMapping<2u>::getLeastSignificantBytIndexForBytesUsedMask(uint32_t bytesUsedMask) const {
	return 31 - _lzcnt_u32(bytesUsedMask);
}

template<typename PartialKeyType> inline PartialKeyType MultiMaskPartialKeyMapping<2u>::getMostSignifikantMaskBit(PartialKeyType mask) const {
	ExtractionDataArray usedExtractionBits = getUsedExtractionBitsForMask(mask);
	uint32_t usedExtractionBytesMask = getBytesUsedMaskForExtractionData(usedExtractionBits);
	uint32_t highestExtractionByteIndex = __tzcnt_u32(usedExtractionBytesMask);
	uint8_t highestByte = getExtractionByteAt(usedExtractionBits, highestExtractionByteIndex);
...
inline uint16_t MultiMaskPartialKeyMapping<2u>::getLeastSignificantBitIndex(uint32_t mask) const {
	ExtractionDataArray usedExtractionBits = getUsedExtractionBitsForMask(mask);
	uint32_t lowestExtractionByteIndex = getLeastSignificantBytIndexForBytesUsedMask(getBytesUsedMaskForExtractionData(usedExtractionBits));
	uint8_t lowestByte = getExtractionByteAt(usedExtractionBits, lowestExtractionByteIndex);
	return convertBytesToBits(getExtractionBytePosition(lowestExtractionByteIndex)) + getLeastSignificantBitIndexInByte(lowestByte);
...
inline uint8_t MultiMaskPartialKeyMapping<2u>::getExtractionBytePosition(unsigned int index) const {
	return reinterpret_cast<uint8_t const *>(mExtractionPositions.data())[index];
}

inline void MultiMaskPartialKeyMapping<2u>::setExtractionBytePosition(unsigned int index, uint8_t extractionByte) {
	reinterpret_cast<uint8_t *>(mExtractionPositions.data())[index] = extractionByte;
}

inline uint8_t MultiMaskPartialKeyMapping<2u>::getExtractionByte(unsigned int index) const {
	return reinterpret_cast<uint8_t const *>(mExtractionData.data())[index];
}

inline void MultiMaskPartialKeyMapping<2u>::setExtractionByte(unsigned int index, uint8_t extractionByte) {
	reinterpret_cast<uint8_t*>(mExtractionData.data())[index] = extractionByte;
}

inline std::set<uint16_t> MultiMaskPartialKeyMapping<2u>::getDiscriminativeBits() const {
	std::set<uint16_t> extractionBits;
	for(int i = 0; i < mNumberExtractionBytes; ++i) {
		unsigned int extractionBasePosition = convertBytesToBits(getExtractionBytePosition(i));
		uint32_t extractionByte = getExtractionByte(i);
...
inline uint16_t MultiMaskPartialKeyMapping<2u>::getNumberExtractionBytes() const {
	return mNumberExtractionBytes;
}

//4u Class Definitions-----------

inline typename MultiMaskPartialKeyMapping<4u>::SIMDRegisterType
MultiMaskPartialKeyMapping<4u>::getPositionsRegister() const {
	return SIMDHelperType::toRegister(mExtractionPositions);
}

inline typename MultiMaskPartialKeyMapping<4u>::SIMDRegisterType
MultiMaskPartialKeyMapping<4u>::getExtractionDataRegister() const {
	return SIMDHelperType::toRegister(mExtractionData);
}

inline void
MultiMaskPartialKeyMapping<4u>::setPositions(typename MultiMaskPartialKeyMapping<4u>::SIMDRegisterType positions) {
	return SIMDHelperType::store(positions, mExtractionPositions);
}

inline void
MultiMaskPartialKeyMapping<4u>::setExtractionData(typename MultiMaskPartialKeyMapping<4u>::SIMDRegisterType extractionData) {
	return SIMDHelperType::store(extractionData, mExtractionData);
}

inline MultiMaskPartialKeyMapping<4u>::MultiMaskPartialKeyMapping(MultiMaskPartialKeyMapping<2u> const & src)
	: PartialKeyMappingBase()
{
	*reinterpret_cast<uint64_t*>(this) = *reinterpret_cast<uint64_t const *>(&src);
...
inline MultiMaskPartialKeyMapping<4u>::MultiMaskPartialKeyMapping(MultiMaskPartialKeyMapping<4u> const & src)
	: PartialKeyMappingBase()
{
	*reinterpret_cast<uint64_t*>(this) = *reinterpret_cast<uint64_t const *>(&src);
...
inline MultiMaskPartialKeyMapping<4u>::MultiMaskPartialKeyMapping(
	SingleMaskPartialKeyMapping const & existing, DiscriminativeBit const & significantKeyInformation
) : PartialKeyMappingBase(existing, significantKeyInformation)
{
	initializeDataAndPositionsWithZero();
...
//detect insert position -> add byte|position -> finish
inline MultiMaskPartialKeyMapping<4u>::MultiMaskPartialKeyMapping(
	MultiMaskPartialKeyMapping<4u> const & existing, DiscriminativeBit const & significantKeyInformation, unsigned int extractionByteIndex
)  : MultiMaskPartialKeyMapping(existing)
{
	//this is a performance optimization to not call the BaseDiscriminativeBitsRepresentation constructor and use copying
	mMostSignificantDiscriminativeBitIndex = std::min(mMostSignificantDiscriminativeBitIndex, significantKeyInformation.mAbsoluteBitIndex);
...
inline MultiMaskPartialKeyMapping<4u>::MultiMaskPartialKeyMapping(
	MultiMaskPartialKeyMapping<4u> const & existing, DiscriminativeBit const & significantKeyInformation, unsigned int const extractionByteIndex,
	typename MultiMaskPartialKeyMapping<4u>::SIMDRegisterType const & maskForLessSignificantBytes
)  : PartialKeyMappingBase(existing, significantKeyInformation), mNumberExtractionBytes(existing.mNumberExtractionBytes + 1), mNumberKeyBits(existing.mNumberKeyBits + 1)
{
	SIMDRegisterType extractionPositionRegister = existing.getPositionsRegister();
	SIMDRegisterType movedPositions = SIMDHelperType::shiftLeftOneByte(SIMDHelperType::binaryAnd(maskForLessSignificantBytes, extractionPositionRegister));
...
template<> inline MultiMaskPartialKeyMapping<4u>::MultiMaskPartialKeyMapping(
	MultiMaskPartialKeyMapping<2u> const & existing, DiscriminativeBit const & significantKeyInformation, unsigned int const extractionByteIndex,
	typename MultiMaskPartialKeyMapping<2u>::SIMDRegisterType const & maskForLessSignificantBytes
...
inline MultiMaskPartialKeyMapping<4u>::MultiMaskPartialKeyMapping(
	...
	uint16_t const mostSignificantBitIndex, uint16_t const leastSignificantBitIndex,
	uint16_t const numberBytesUsed, uint16_t const bitsUsed
) : MultiMaskPartialKeyMapping<4u>(mostSignificantBitIndex, leastSignificantBitIndex, numberBytesUsed, bitsUsed) {
	assert(_mm_popcnt_u32(bitsUsed) >= 1);
...
inline __attribute__((always_inline)) uint32_t MultiMaskPartialKeyMapping<4u>::extractMask(uint8_t const * keyBytes) const {
	return extractMaskForMappedInput(mapInput(keyBytes));
}
 ...
 * Extracts a bit masks corresponding to 111111111|00000000 with the first zero bit marking the misssmatching bit index.
 */
template<typename PartialKeyType> inline PartialKeyType MultiMaskPartialKeyMapping<4u>::getPrefixBitsMask(DiscriminativeBit const &significantKeyInformation) const {
	//initialized to zero
	ExtractionDataArray prefixBits = zeroInitializedArray();
	uint8_t subtreeMissmatchingByte = static_cast<uint8_t >(UINT8_MAX) << (8 - significantKeyInformation.mByteRelativeBitIndex);
...
inline uint16_t MultiMaskPartialKeyMapping<4u>::calculateNumberBitsUsed() const {
	return mNumberKeyBits;
}

template<typename Operation> inline auto MultiMaskPartialKeyMapping<4u>::insert(DiscriminativeBit const & significantKeyInformation, Operation const & operation) const {
	SIMDRegisterType simdMask = getMaskForPositionsLargerOrEqualTo(significantKeyInformation.mByteIndex);
	//| (UINT32_MAX << mNumberExtractionBytes) is important here. it corresponds to all bytes which are currently unused. Hence, they are expected to be larger
	uint32_t maskForPositionsLargerOrEqual = SIMDHelperType::moveMask8(simdMask) | (UINT32_MAX << mNumberExtractionBytes);
	unsigned int possibleExtractionByteIndex = __tzcnt_u32(maskForPositionsLargerOrEqual);
	...
	constexpr unsigned int nextNumberExtractionMasks = 4u == 4 ? 4u : 4u * 2;
	return (isByteContainedAtPosition)
		? operation(MultiMaskPartialKeyMapping<4u>(*this, significantKeyInformation, possibleExtractionByteIndex))
		: (mNumberExtractionBytes < (4u * 8)
		   ? operation(MultiMaskPartialKeyMapping<4u>(*this, significantKeyInformation, possibleExtractionByteIndex, simdMask))
		   : operation(MultiMaskPartialKeyMapping<nextNumberExtractionMasks>(*this, significantKeyInformation, possibleExtractionByteIndex, simdMask))
		  );
}

template<typename Operation>
inline auto MultiMaskPartialKeyMapping<4u>::extract(uint32_t bitsUsed, Operation const &operation) const {
	assert(_mm_popcnt_u32(bitsUsed) >= 1);
	...
	assert((numberRandomExtractionMasksMinusOne + isRandomExtractionMask) <= 4u);
	assert((numberRandomExtractionMasksMinusOne + isRandomExtractionMask) <= 4);
	switch(numberRandomExtractionMasksMinusOne + isRandomExtractionMask) {
		case 0:
			return operation(SingleMaskPartialKeyMapping(
		...
		case 1:
			return operation(MultiMaskPartialKeyMapping<4u>(
				(MultiMaskPartialKeyMapping<4u> const &)(*this), bytesUsedMask, extractionDataForBitsUsed,
				mostSignificantBitPosition, leastSignificantBitPosition, numberBytesUsed, static_cast<uint16_t>(_mm_popcnt_u32(bitsUsed))
			));
		case 2:
			if(4u >= 2) {
...
inline std::array<uint8_t, 256> MultiMaskPartialKeyMapping<4u>::createIntermediateKeyWithOnlySignificantBitsSet() const {
	std::array<uint8_t, 256> intermediateKey;
	std::memset(intermediateKey.data(), 0, 256);
	for(size_t i=0u; i < mNumberExtractionBytes; ++i) {
		intermediateKey[getExtractionBytePosition(i)] = getExtractionByte(i);
...
template<typename Operation> inline auto MultiMaskPartialKeyMapping<4u>::executeWithCorrectMaskAndDiscriminativeBitsRepresentation(Operation const & operation) const {
	return operation(*this, static_cast<uint32_t>(UINT32_MAX));
}

template<typename Operation> inline auto MultiMaskPartialKeyMapping<4u>::executeWithCompressedDiscriminativeBitsRepresentation(Operation const & operation) const {
	uint8_t leasSignificantExtractionBytePosition = getExtractionBytePosition(getNumberExtractionBytes() - 1);
	uint8_t mostSignificantExtractionBytePosition = getExtractionBytePosition(0);
	size_t extractionByteRange = leasSignificantExtractionBytePosition - mostSignificantExtractionBytePosition;
	if(extractionByteRange < 8) {
...
inline __m256i MultiMaskPartialKeyMapping<4u>::getMaskForPositionsLargerOrEqualTo(unsigned int bytePosition) const {
...
inline void MultiMaskPartialKeyMapping<4u>::initializeDataAndPositionsWithZero() {
...
inline typename MultiMaskPartialKeyMapping<4u>::ExtractionDataArray MultiMaskPartialKeyMapping<4u>::mapInput(uint8_t const* __restrict__ keyBytes) const {
	ExtractionDataArray mappedInput = zeroInitializedArray();
	uint8_t* __restrict__ mappedInputBytes = reinterpret_cast<uint8_t*>(mappedInput.data());
	uint8_t const * __restrict__ positions = reinterpret_cast<uint8_t const* >(mExtractionPositions.data());
	for(int i=0; i < mNumberExtractionBytes; ++i) {
...
inline __attribute__((always_inline)) uint32_t MultiMaskPartialKeyMapping<4u>::extractMaskForMappedInput(typename MultiMaskPartialKeyMapping<4u>::ExtractionDataArray const & mappedInputData) const {
...
inline uint32_t  MultiMaskPartialKeyMapping<4u>::getMaskForHighestBit() const {
	unsigned int byteRelativeBitPositionForHighestBit = mMostSignificantDiscriminativeBitIndex - convertBytesToBits(getExtractionBytePosition(0));
...
inline uint32_t  MultiMaskPartialKeyMapping<4u>::getMaskFor(DiscriminativeBit const & significantKeyInformation) const {
	ExtractionDataArray extractionData = zeroInitializedArray();
	setExtractionByteAt(
		extractionData,
		getExtractionByteIndexForPosition(significantKeyInformation.mByteIndex),
...
inline uint32_t  MultiMaskPartialKeyMapping<4u>::getAllMaskBits() const {
	uint32_t numberUnusedBits = (32 - mNumberKeyBits);
	return UINT32_MAX >> numberUnusedBits;
}

inline uint32_t  MultiMaskPartialKeyMapping<4u>::getExtractionByteIndexForPosition(uint16_t bytePosition) const {
	SIMDRegisterType needle = SIMDHelperType::set1_epi8(bytePosition);
	SIMDRegisterType haystack = getPositionsRegister();
	uint32_t bytePositionMask = SIMDHelperType::moveMask8(SIMDHelperType::cmpeq_epi8(needle, haystack)) & getMaskForExtractionBytesUsed();
	assert(bytePositionMask != 0);
...
inline uint8_t  MultiMaskPartialKeyMapping<4u>::getExtractionByteWithBitSetAtRelativePosition(uint16_t byteRelativeBitPosition) {
	return BYTE_WITH_HIGHEST_BIT_SET >> byteRelativeBitPosition;
}

inline typename MultiMaskPartialKeyMapping<4u>::ExtractionDataArray MultiMaskPartialKeyMapping<4u>::getUsedExtractionBitsForMask(uint32_t usedMaskBits) const {
...
inline typename MultiMaskPartialKeyMapping<4u>::ExtractionDataArray MultiMaskPartialKeyMapping<4u>::zeroInitializedArray() {
	ExtractionDataArray data;
	_mm256_storeu_si256(reinterpret_cast<__m256i*>(data.data()), _mm256_setzero_si256());
	return std::move(data);
}

inline uint8_t MultiMaskPartialKeyMapping<4u>::getExtractionByteAt(typename MultiMaskPartialKeyMapping<4u>::ExtractionDataArray const &extractionData, uint32_t extractionByteIndex) {
	return reinterpret_cast<uint8_t const *>(extractionData.data())[extractionByteIndex];
}


inline void MultiMaskPartialKeyMapping<4u>::setExtractionByteAt(typename MultiMaskPartialKeyMapping<4u>::ExtractionDataArray  & extractionData, uint32_t extractionByteIndex, uint8_t extractionByte) {
	reinterpret_cast<uint8_t *>(extractionData.data())[extractionByteIndex] = extractionByte;
}

inline uint32_t MultiMaskPartialKeyMapping<4u>::getMaskForExtractionBytesUsed() const {
	return (UINT32_MAX >> (32 - mNumberExtractionBytes));
}

inline uint32_t MultiMaskPartialKeyMapping<4u>::getBytesUsedMaskForExtractionData(typename MultiMaskPartialKeyMapping<4u>::ExtractionDataArray const & extractionData) const {
	return ~SIMDHelperType::moveMask8(SIMDHelperType::cmpeq_epi8(SIMDHelperType::toRegister(extractionData), SIMDHelperType::zero())) & getMaskForExtractionBytesUsed();
}

inline uint32_t MultiMaskPartialKeyMapping<4u>::getLeastSignificantBytIndexForBytesUsedMask(uint32_t bytesUsedMask) const {
	return 31 - _lzcnt_u32(bytesUsedMask);
}

template<typename PartialKeyType> inline PartialKeyType MultiMaskPartialKeyMapping<4u>::getMostSignifikantMaskBit(PartialKeyType mask) const {
	ExtractionDataArray usedExtractionBits = getUsedExtractionBitsForMask(mask);
	uint32_t usedExtractionBytesMask = getBytesUsedMaskForExtractionData(usedExtractionBits);
	uint32_t highestExtractionByteIndex = __tzcnt_u32(usedExtractionBytesMask);
	uint8_t highestByte = getExtractionByteAt(usedExtractionBits, highestExtractionByteIndex);
...
inline uint16_t MultiMaskPartialKeyMapping<4u>::getLeastSignificantBitIndex(uint32_t mask) const {
	ExtractionDataArray usedExtractionBits = getUsedExtractionBitsForMask(mask);
	uint32_t lowestExtractionByteIndex = getLeastSignificantBytIndexForBytesUsedMask(getBytesUsedMaskForExtractionData(usedExtractionBits));
	uint8_t lowestByte = getExtractionByteAt(usedExtractionBits, lowestExtractionByteIndex);
	return convertBytesToBits(getExtractionBytePosition(lowestExtractionByteIndex)) + getLeastSignificantBitIndexInByte(lowestByte);
...
inline uint8_t MultiMaskPartialKeyMapping<4u>::getExtractionBytePosition(unsigned int index) const {
	return reinterpret_cast<uint8_t const *>(mExtractionPositions.data())[index];
}

inline void MultiMaskPartialKeyMapping<4u>::setExtractionBytePosition(unsigned int index, uint8_t extractionByte) {
	reinterpret_cast<uint8_t *>(mExtractionPositions.data())[index] = extractionByte;
}

inline uint8_t MultiMaskPartialKeyMapping<4u>::getExtractionByte(unsigned int index) const {
	return reinterpret_cast<uint8_t const *>(mExtractionData.data())[index];
}

inline void MultiMaskPartialKeyMapping<4u>::setExtractionByte(unsigned int index, uint8_t extractionByte) {
	reinterpret_cast<uint8_t*>(mExtractionData.data())[index] = extractionByte;
}

inline std::set<uint16_t> MultiMaskPartialKeyMapping<4u>::getDiscriminativeBits() const {
	std::set<uint16_t> extractionBits;
	for(int i = 0; i < mNumberExtractionBytes; ++i) {
		unsigned int extractionBasePosition = convertBytesToBits(getExtractionBytePosition(i));
		uint32_t extractionByte = getExtractionByte(i);
...
inline uint16_t MultiMaskPartialKeyMapping<4u>::getNumberExtractionBytes() const {
	return mNumberExtractionBytes;
}
}}
